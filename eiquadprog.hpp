    ...
    if (t == t2)
    {
        ...
        if (!add_constraint(R, J, d, iq, R_norm))
        {
            ...
            for (i = 0; i < iq; i++)
            {
                ...
                /* 
                 * XXX
                 * It seems that there is a bug in handling of degenerate
                 * cases. Element A(i) can be a negative number, which cannot
                 * be used as an index. To avoid abortion I return NaN here.
                 * XXX
                 */
                if (A(i) < 0)
                {
