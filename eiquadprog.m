% [x, info] = eiquadprog (H)
% [x, info] = eiquadprog (H, g)
% [x, info] = eiquadprog (H, g, A, b)
% [x, info] = eiquadprog (H, g, A, b, lb, ub)
% [x, info] = eiquadprog (H, g, A, b, lb, ub, Ain, lbin, ubin)
% [x, info] = eiquadprog (..., options)
%
...
function [x, info] = eiquadprog(varargin)
    H = [];
    g = [];
    A = [];
    b = [];
...
% Solve
    ...
    [x, info] = eiquadprog_interface(H + eye(size(H))*regularization_factor, g, A, b, Ain_tmp, bin_tmp);
end
